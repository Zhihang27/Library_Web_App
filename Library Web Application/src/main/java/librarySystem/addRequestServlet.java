...
public class addRequestServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int status = RequestDAO.save(r);
        if (status > 0) {
            out.print("<p>Request submitted successfully!</p>");
            request.getRequestDispatcher("Request_Book_Form.html").include(request, response);
        } else {
            out.println("Sorry, unable to submit the book request.");
        }
